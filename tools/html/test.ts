function downloadFile(url: string) : Promise<Uint8Array> {
...
interface CanvasRenderingContext2D {
    put_image_data(point : Point, imagedata: ImageData)
}
...
function tile_index_to_position(tile_index : number) {
    return {
        x: (~~(tile_index % 20)) * 16,
        y: (~~(tile_index / 20)) * 10,
    }
...
function mat_content_to_image_data(pal: Palette, mat : Uint8Array, offset : number, flip: boolean) {
...
function map_content_to_tiles(mat: Uint8Array, pal: Palette):Tile[] {
    var tiles : Tile[] = []
...
function create_image_element(imagedata : ImageData) : HTMLCanvasElement {
...
function create_tileset_canvas() : HTMLCanvasElement {
    let canvas = document.createElement("canvas")
    canvas.width = 320
    canvas.height = 200
...
function create_and_draw_tileset_canvas(tileset : ImageData[]) {
...
class Room {
    ...
    constructor(data: Uint8Array, nr: number) {
        this.data = data
        this.nr = nr
    }
    ...
    tilemap_at(index: number) {
        return (this.data[this.offset() + 0x04 + index * 2 + 1])
    }

    tileid_at(index: number) {
        return (
            (this.data[this.offset() + 0x24 + index * 2    ] << 8) + 
            (this.data[this.offset() + 0x24 + index * 2 + 1] << 0)
        )
    }
}

function parse_room(roomdata) : Room[] {
    let rooms = []
...
function draw_room(room : Room, tilesets : Map<number, Tile[]>, ctx : CanvasRenderingContext2D) {
...
function css_px(value: number, multiplier: number) : string {
...
window.onload = () => {
    ...
    .then((files) => {
        ...
        let tilesets_tuples  = buffers.map((tileset) => {
            return [tileset.nr, map_content_to_tiles(tileset.data, palette)] as [number, Tile[]]
        })
        ...
        canvas_container.onmousemove = (ev) => {
            ...
            room_bottom.innerHTML = (
                "Mouse: " + JSON.stringify(mousePoint) + "<br>" 
